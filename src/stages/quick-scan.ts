...
function renderHtml(_runId: string, assessment: ReturnType<typeof buildAssessment>): string {
  const actions = assessment.rankedActions;
  const repo = escapeHtml(repositoryName(assessment));
  ...
  const coverageRows = assessment.coverage
    .map(
      (entry) =>
        `<tr><td>${escapeHtml(coverageCheckLabel(entry.check))}</td><td><span class="status status--${coverageStatusClass(
          entry.status,
        )}">${escapeHtml(coverageStatusLabel(entry.status))}</span></td><td>${escapeHtml(
          entry.reason ?? "",
        )}</td></tr>`,
    )
    .join("");
  const fixWord = actions.length === 1 ? "fix to make" : "fixes to make";
  ...
  return [
    ...
    "<main>",
    '<header class="head">',
    '<div class="brand"><span class="dot"></span>VibeShield <span class="brand-sub">Quick Scan</span></div>',
    `<h1>${repo}</h1>`,
    "</header>",
    ...
    "</section>",
    '<div class="stats">',
    `<div class="stat"><b>${actions.length}</b><span>${fixWord}</span></div>`,
    `<div class="stat"><b>${assessment.manifest.fileCount}</b><span>files scanned</span></div>`,
    "</div>",
    `<p class="note">${escapeHtml(assessment.limitation)}</p>`,
    "<h2>Fix these first</h2>",
    actions.length > 0
      ? '<p class="lede">Each problem below comes with a prompt you can paste straight into your coding agent. Work through them in order.</p>'
      : '<p class="lede">No blocking fixes were produced by the checks that completed.</p>',
    actionCards,
    ...
    "</details>",
    `<footer class="foot">${escapeHtml(footerMeta)}</footer>`,
    "</main>",
...
function renderActionHtml(
  ...
  const ops =
    remediation.operationalSteps.length > 0
      ? `<div class="ops"><strong>You'll need to do this yourself (your agent can't):</strong><ul class="steps">${remediation.operationalSteps
...
function verdictHtmlClass(verdict: ReturnType<typeof buildAssessment>["verdict"]): string {
...
function verdictSubline(assessment: ReturnType<typeof buildAssessment>): string {
...
function repositoryName(assessment: ReturnType<typeof buildAssessment>): string {
  const name = assessment.repository.name.trim();
  return name.length > 0 ? name : "this repository";
}

function coverageCheckLabel(check: string): string {
...
const REPORT_CSS = [
  ...
  ".meta{color:var(--muted);font-size:14px;margin:0;word-break:break-all}",
  ...
  "ul.steps{margin:0;padding-left:20px}ul.steps li{margin:3px 0}",
...
function renderHtmlList(heading: string, values: ReadonlyArray<string>): string {
  ...
  return `<h4>${escapeHtml(heading)}</h4><ul class="steps">${items}</ul>`;
}
