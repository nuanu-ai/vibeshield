...
vulnerable application. Each check family needs its own benchmark source, truth
oracle, metric, and overfit check. It applies to both R&D phases:
...
- build the benchmark matrix by check family, not by one repository list;
- include lanes for secrets, vulnerable dependencies, CI/CD, IaC/config,
  deterministic code patterns, Deep Static taint/injection, static hypotheses,
  mixed-language coverage, and deploy-blocking verdicts;
- pin origin, commit, tool versions, and vulnerability database freshness;
- curate or generate ground truth for every scored lane;
...
- a lane does not enter scored precision/recall until its truth oracle is
  curated, generated by fixture construction, or otherwise externally pinned;
...
such as WebGoat and Juice Shop are useful for the Deep Static taint/injection
lane and end-to-end regression, but they are not a fair benchmark for the whole
product because they under-test secrets, dependency, CI/CD, and IaC coverage and
over-represent runtime/business-logic vulnerabilities.

| Check family | Primary benchmark source | Truth oracle | Scored surface |
| --- | --- | --- | --- |
| Secrets | Seeded repositories with known fake secrets and clean controls | The seeded secret manifest plus scanner rule identity | Direct findings |
| Vulnerable dependencies | Pinned npm/pip/go manifests with known vulnerable and patched versions | Manifest/CVE pairs, vulnerability database snapshot, and duplicate advisory review | Direct findings and dependency usage coverage |
| CI/CD | GitHub Actions fixtures for actionlint/zizmor-relevant patterns | Intentionally bad workflow cases and clean controls | Direct findings |
| IaC/config | Terraform, CloudFormation, Kubernetes, and Kustomize goat-style fixtures | Scanner rule catalog plus curated review of additional true findings | Direct findings |
| Deterministic code patterns | Small language-specific vulnerable/clean fixtures | Fixture construction and source-pinned vulnerable lines | Direct findings |
| Deep Static taint/injection | OWASP-style SAST benchmarks plus static-detectable slices of WebGoat, Juice Shop, Vulnerable-Flask-App, and go-dvwa | Source-to-sink labels and static-detectability markings | Static hypotheses and direct findings where emitted |
| Static hypotheses support | Pinned supported-path reports and reviewed extra traces | Curated expected paths, true-but-uncurated buckets, and false-support review | Support precision and candidate recall |
| Mixed-language coverage | Mixed-language fixtures and canary repositories | Expected language inventory and coverage states | Coverage gate |
| Deploy verdict | Curated deploy-blocking repositories across families | Expected `not-ready-to-deploy` verdict | Verdict correctness |

Each lane must have tuning and held-out inputs before its metric can contribute
to the Phase 1 success claim. A repository can contribute to multiple lanes, but
no single training app should define the product benchmark.

## Ground Truth

Scored quality is measured only against curated external truth, not against
VibeShield's own output. Truth sources are lane-specific:

- fixture construction for seeded secrets, deterministic vulnerable code, and
  clean controls;
- pinned manifest/CVE pairs for dependency lanes;
- intentionally vulnerable workflow or IaC fixtures plus scanner rule catalog
  expectations where the scanner has a well-defined rule surface;
- externally documented SAST benchmarks and static-detectability markings for
  source-to-sink taint lanes;
- curated repository truth for intentionally vulnerable applications when the
  vulnerability is visible to static analysis.
...
Current curated Deep Static expectations exist for WebGoat, Juice Shop,
Vulnerable-Flask-App, and go-dvwa. They are the first taint/injection lane, not
the whole product benchmark. The next methodology gate is to add scored family
lanes for secrets, dependencies, CI/CD, and IaC/config before claiming that the
Phase 1 target measures the product surface.
...
positives. For IaC/config in particular, an author's intended-vulnerability list
is not enough to declare all other scanner results false; the scanner's rule
catalog and resource evidence must be part of the truth oracle.
...
They apply per scored family and, where code language matters, per language for
JS/TS, Python, Go, and Java. A target is not achieved until every required lane
is scoreable, has tuning and held-out coverage, and meets the threshold without
